...
function M.mixin(cls)
    ...
    function cls:_getKnownFiles(dirPath)
        local fileMap = {
            ...
            ["image/card"] = {
                "card_F01_烈焰弹_20260601140000.png",
                "card_I01_冰霜刺_20260601140000.png",
                "card_T01_雷霆击_20260601140000.png",
                "card_W01_旋风斩_20260601140000.png",
                "card_E02_尖刺陷阱_20260601140000.png",
            },
    ...
    function cls:_restoreEditorRoot()
        local UI = require("urhox-libs/UI")
        if self._savedRoot then
            UI.SetRoot(self._savedRoot)
            self._savedRoot = nil
        end
    end
    ...
    function cls:_openCardPoolEditor()
        if self.cardPoolOpen then return end
        self.cardPoolOpen = true

        local CardData = require("card_data")

        -- 按元素分组排列卡牌
        local elementOrder = { "fire", "ice", "wind", "thunder", "earth", "time", "slow", "space", "matter", "none" }
        local sortedCards = {}
        for _, card in pairs(CardData.CARDS) do
            table.insert(sortedCards, card)
        end
        table.sort(sortedCards, function(a, b)
            local ai, bi = 99, 99
            for i, e in ipairs(elementOrder) do
                if a.element == e then ai = i end
                if b.element == e then bi = i end
            end
        ...
        for _, card in ipairs(sortedCards) do
            -- 元素分组标题
            if card.element ~= currentElement then
                currentElement = card.element
                local elemInfo = CardData.ELEMENT_COLORS[card.element] or { icon = "?", name = card.element }
                table.insert(cardRows, UI.Label {
                    text = elemInfo.icon .. " " .. elemInfo.name .. "属性",
                    fontSize = 12,
                    fontColor = { 255, 220, 100, 255 },
                    fontWeight = "bold",
                    marginTop = #cardRows > 0 and 8 or 0,
                    marginBottom = 2,
                })
            end

            -- 计算概率
            local pct = totalWeight > 0 and (card.weight / totalWeight * 100) or 0

            -- 卡牌行: 名称 + 稀有度 + 权重滑块 + 概率百分比
            local rarityColor = card.rarity == "rare" and {255,180,50,255} or {180,180,200,255}
            local rarityText = card.rarity == "rare" and "[稀]" or "[普]"

            table.insert(cardRows, UI.Panel {
                width = "100%",
                flexDirection = "row",
                gap = 4,
                alignItems = "center",
                ...
                children = {
                    ...
                    UI.Slider {
                        ...
                        step = 1,
                        onChange = function(_, v)
                            self:_onCardWeightChanged(card.id, v)
                        end,
                    },
                    -- 权重数值
                    UI.Label {
                        id = "card_wval_" .. card.id,
                        text = tostring(card.weight),
                        fontSize = 10,
                        fontColor = {200,200,230,255},
                        width = 18,
                        textAlign = "center",
        ...
        -- 汇总信息
        local summaryText = string.format("共 %d 张卡牌，总权重 %d", #sortedCards, totalWeight)

        self.cardPoolPanel = UI.Panel {
            id = "card_pool_editor",
            position = "absolute",
            left = "15%",
            top = "5%",
            ...
            children = {
                ...
                UI.Panel {
                    ...
                    children = {
                        ...
                        UI.Button {
                            ...
                            width = 60, height = 28,
                            variant = "danger",
                            onClick = function() self:_closeCardPoolEditor() end,
                        },
                    },
                },
                -- 说明
                UI.Label {
                    text = "拖动滑块调整卡牌权重（0=不出现，越大越容易抽到）",
                    fontSize = 10,
                    fontColor = { 160, 160, 190, 255 },
                    marginBottom = 4,
                },
                ...
                UI.Panel {
                    ...
                    marginBottom = 4,
                    children = {
                        UI.Label { text = "卡牌名", fontSize = 10, fontColor = {150,150,180,255}, width = 90 },
                        UI.Label { text = "权重", fontSize = 10, fontColor = {150,150,180,255}, width = 80, textAlign = "center" },
                        UI.Label { text = "值", fontSize = 10, fontColor = {150,150,180,255}, width = 18, textAlign = "center" },
                        UI.Label { text = "概率", fontSize = 10, fontColor = {150,150,180,255}, width = 40, textAlign = "right" },
                    },
                },
                -- 卡牌列表
                table.unpack(cardRows),
            },
        }
        self._savedRoot = UI.GetRoot()
        UI.SetRoot(self.cardPoolPanel)
    end

    function cls:_closeCardPoolEditor()
        self.cardPoolOpen = false
        if self.cardPoolPanel then
            self.cardPoolPanel:Hide()
            self.cardPoolPanel = nil
        end
        -- 恢复编辑器主UI根
        self:_restoreEditorRoot()
    end

    function cls:_onCardWeightChanged(cardId, newWeight)
        local CardData = require("card_data")
        local card = CardData.CARDS[cardId]
        if not card then return end

        -- 更新权重
        card.weight = math.floor(newWeight)

        -- 重新计算总权重
        local totalWeight = 0
        for _, c in pairs(CardData.CARDS) do
            totalWeight = totalWeight + c.weight
        end

        -- 重建卡池
        CardData.POOL = {}
        for id, c in pairs(CardData.CARDS) do
            for i = 1, c.weight do
        ...
        if self.cardPoolPanel then
            -- 更新当前卡牌的数值
            local valLbl = self.cardPoolPanel:FindById("card_wval_" .. cardId)
            if valLbl then valLbl:SetText(tostring(card.weight)) end

            -- 更新所有卡牌的概率百分比
            for id, c in pairs(CardData.CARDS) do
                local pctLbl = self.cardPoolPanel:FindById("card_pct_" .. id)
                if pctLbl then
                    local pct = totalWeight > 0 and (c.weight / totalWeight * 100) or 0
                    pctLbl:SetText(string.format("%.1f%%", pct))
                end
            end

            -- 更新汇总信息
            local summaryLbl = self.cardPoolPanel:FindById("card_pool_summary")
            if summaryLbl then
                local cardCount = 0
                for _ in pairs(CardData.CARDS) do cardCount = cardCount + 1 end
                summaryLbl:SetText(string.format("共 %d 张卡牌，总权重 %d", cardCount, totalWeight))
            end
        end
    end
