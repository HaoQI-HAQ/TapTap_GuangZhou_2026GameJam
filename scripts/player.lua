...
local ATK_OFFSET_Y = 0.17  -- 攻击时精灵上移0.15米
...
function Player:new(scene, inputManager)
    ...
    self.jumpGraceTimer = 0  -- 跳跃后短暂禁止地面检测重置
...
function Player:update(dt)
    ...
    if self.slamming then
        -- 先读取物理解算后的实际速度（上帧 SetLinearVelocity 经过碰撞解算后的结果）
        ...
        if stompedEnemy then
            ...
            if centerOnGround then
                -- 中心在平台上，正常落地
                self:_slamLand(false)
            else
                -- 中心不在平台上：检测是否被边缘部分阻挡
                -- 如果实际下落速度比设定值慢了超过30%，说明有碰撞在阻挡（边缘接触）
                local blocked = (vel.y > -SLAM_SPEED * 0.7) and (self.slamFrames or 0) >= 2
                ...
                if blocked then
                    ...
                    local slideDir = 0
                    if leftSolid and not rightSolid then
                        slideDir = 1
                    elseif rightSolid and not leftSolid then
                        slideDir = -1
                    else
                        slideDir = self.facingRight and 1 or -1
                    end
                    self.body:SetLinearVelocity(Vector2(slideDir * 6.0, -SLAM_SPEED * 0.5))
                end
            end
        end
        self.slamFrames = (self.slamFrames or 0) + 1
        self:_updateAnimation(dt)
        return
    end
    ...
    -- 着地检测：从玩家脚底向下射线检测（起跳保护期间不重置）
    self.isGrounded = self:_checkGrounded()
    if self.isGrounded and self.jumpGraceTimer <= 0 then
        self.jumpCount = 0
    end

    -- 跳跃（按下瞬间触发，限制2段：地面1跳+空中1跳）
    ...
    if jumpAction and not self.jumpPressed then
        if self.jumpCount < MAX_JUMPS then
            ...
            self.jumpGraceTimer = 0.15  -- 起跳后0.15秒内不重置jumpCount
        end
    end
...
function Player:_slamLand(fullCircle)
    ...
    if self.collisionShapes then
        self.collisionShapes[1].friction = 0.3  -- 中段矩形
        self.collisionShapes[2].friction = 0.3  -- 顶部圆形
        self.collisionShapes[3].friction = 0.0  -- 底部圆形保持零摩擦
    end
...
function Player:_updateDeath(dt)
    ...
    -- 动画播完后触发 Game Over（只触发一次）
    if self.deathTimer >= DIE_ANIM_DURATION + 0.5 then
        if self.gameOverCallback and not self.gameOverTriggered then
...
-- 检测角色在平台边缘的支撑比例（0.0~1.0）
-- 发射5条等距射线覆盖碰撞体宽度，返回命中比例
function Player:_getGroundSupport()
    local pos = self.node.position
    local halfW = 0.25  -- 碰撞体半宽
    local rayY = pos.y - 1.0
    local rayEndY = pos.y - 1.2  -- 稍长一点确保检测到
    local hits = 0
    local rays = 5
