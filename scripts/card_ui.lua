...
local CARD_RIGHT_MARGIN = 24
local CARD_BOTTOM_OFFSET = 140  -- 攻击按钮上方（btn底部30+高88+间隙22≈140）
...
function CardUI:_setup()
    ...
    -- 缓存缩放后的尺寸（使用交互控件缩放）
    self._cw = U(CARD_WIDTH)
    self._ch = U(CARD_HEIGHT)
    self._cs = U(CARD_SPACING)
    ...
    self.container:SetPosition(-U(CARD_RIGHT_MARGIN), -U(CARD_BOTTOM_OFFSET))
